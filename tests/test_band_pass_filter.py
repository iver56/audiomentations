...
class TestBandPassFilter(unittest.TestCase):
    def test_band_pass_filter(self):
        ...
        augment = BandPassFilter( min_center_freq=100.0, max_center_freq=1000.0, min_q=1.0, max_q=2.0, p=1.0)
        ...
        samples_db_at_fcL = 10*np.log10(samples_pxx[int(np.round(nfft/sample_rate * fcL))])
        processed_samples_db_at_fcL = 10*np.log10(processed_samples_pxx[int(np.round(nfft/sample_rate * fcL))])

        samples_db_at_fcH = 10*np.log10(samples_pxx[int(np.round(nfft/sample_rate * fcH))])
        processed_samples_db_at_fcH = 10*np.log10(processed_samples_pxx[int(np.round(nfft/sample_rate * fcH))])        
        ...
        self.assertTrue(np.isclose(samples_db_at_fcL - processed_samples_db_at_fcL, 3, 0.5))
        self.assertTrue(np.isclose(samples_db_at_fcH - processed_samples_db_at_fcH, 3, 0.5))
