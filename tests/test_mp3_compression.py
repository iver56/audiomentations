...
class TestMp3Compression(unittest.TestCase):
    def test_apply_mp3_compression(self):
        ...
        sample_rate = 44100
        augmenter = Compose([Mp3Compression(p=1.0, min_bitrate=48, max_bitrate=48)])
    ...
    def test_apply_mp3_compression_low_bitrate(self):
        sample_len = 16000
        samples_in = np.random.normal(0, 1, size=sample_len).astype(np.float32)
        sample_rate = 16000
        augmenter = Compose([Mp3Compression(p=1.0, min_bitrate=8, max_bitrate=8)])
