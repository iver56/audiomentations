...
class TestShift(unittest.TestCase):
    def test_shift(self):
        ...
        forward_augmenter = Compose([Shift(min_amount=0.5, max_amount=0.5, p=1.0)])
        ...
        backward_augmenter = Compose([Shift(min_amount=-0.25, max_amount=-0.25, p=1.0)])
        backward_shifted_samples = backward_augmenter(
            samples=samples, sample_rate=sample_rate
        )
        assert_almost_equal(
            backward_shifted_samples, np.array([0.5, 0.25, 0.125, 1.0], dtype=np.float32)
        )
